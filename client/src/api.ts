...
export const usesStaticDemoData = staticDemoMode;
const api = axios.create({ baseURL: import.meta.env.VITE_API_BASE_URL || '/api', timeout: 45_000 });
...
export async function getHistory(symbol: string, market: 'TWSE' | 'TPEx', from: string, to: string): Promise<ApiResponse<HistoryData>> {
  ...
  return (await api.get<ApiResponse<HistoryData>>(`/stocks/${symbol}/history`, { params: { market, from, to }, timeout: 45_000 })).data;
}
